...
class Agent:
    ...
    def max(self, current_board, current_color, depth, alpha, beta):
        ...
        for move in possible_moves:
            ...
            if (value > best_move_value):
                ...
                if alpha < best_move_value:
                    alpha = best_move_value
                if beta <= best_move_value:
    ...
    def min(self, current_board, current_color, depth, alpha, beta):
        ...
        for move in possible_moves:
            temp_move, value = self.max(current_board.next_board(current_color, move), self.game.opponent(current_color), depth + 1, alpha, beta)
            if (value < best_move_value):
                best_move_value = value
                best_move = move
